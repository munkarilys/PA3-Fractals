...
Fractal3D::Fractal3D(string name, int min_level, int actual_level, int max_level, ofEasyCam* cam) : AbstractFractal(name), cam(cam) {
        this->limit = max_level;
        this->min_limit = min_level;
        this->fractal_level = actual_level;
}
...
void Fractal3D::draw(){
    Fractal3D::reset();
    draw((map<string, float> {{"n", getLevel()}, {"scale", 100}}));
}
